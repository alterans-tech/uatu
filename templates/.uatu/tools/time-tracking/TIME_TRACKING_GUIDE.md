# Claude Code Time Tracking Guide

This guide explains how to analyze time spent working with Claude Code on any project.

## Quick Start: Ask Claude

The easiest way to check your work time is to **ask Claude directly**:

```
"How much time have I worked today?"
"Show me my work time for this project"
"What's my worklog?"
"How many hours this week?"
```

Claude will run the worklog tool and show you the results. No need to run scripts manually.

**More example prompts:**
- "Check my work hours"
- "Show today's work sessions"
- "How long have I been working on this?"
- "What's my total time on this project?"
- "Show work blocks for January 27"

---

## Overview

Claude Code stores session data in `~/.claude/projects/` with a folder for each project path. The folder name is the project path with `/` replaced by `-`.

Example:
- Project path: `/Users/arnaldosilva/Workspace/projects/toptal/globtech`
- Session folder: `~/.claude/projects/-Users-arnaldosilva-Workspace-projects-toptal-globtech/`

## Data Format

Session files are `.jsonl` (JSON Lines) with entries like:

```json
{"type": "user", "timestamp": "2026-01-27T10:30:00.000Z", "message": {...}}
{"type": "assistant", "timestamp": "2026-01-27T10:30:05.000Z", "message": {...}}
```

**Important distinctions:**
- `type: "user"` includes both real user prompts AND tool results (when Claude uses tools)
- Tool results have `content` containing `tool_use_id` - these are NOT real prompts
- Timestamps are in UTC - convert to local timezone for display

## Methodology

### Work Time Calculation

1. **Timeout threshold: 10 minutes**
   - Gap ≤ 10 min → Count as work time (user still engaged)
   - Gap > 10 min → Don't count (user took a break)

2. **Work blocks**
   - Continuous activity separated by gaps > 10 min
   - Duration = time from first event to last event in block

3. **Filtering real prompts**
   - Only count messages where `type: "user"` AND content is NOT a tool result
   - Tool results have `{"tool_use_id": "..."}` in their content

### Metrics

- **Duration**: Total work time in the block (sum of gaps ≤ 10 min)
- **Prompts**: Number of real user prompts (excluding tool results)
- **Avg Gap**: Average time between user prompts

## Python Script

Save this as `worklog.py` and run with:

```bash
python worklog.py --project /path/to/your/project --tz -3
```
...
```bash
python worklog.py --project /Users/me/myproject --tz -3 --day 2026-01-27
```

### Find your project's session folder
...
#   Started   Stopped   Duration                          Prompts   Avg Gap
1   06:56     07:06     10 min   █                              4         3.2m
2   08:55     09:38     43 min   █████                          13        3.3m
3   10:52     12:30     99 min   ████████████                   24        4.2m

Day total: 152 min (2.5h) | Window: 06:56 → 12:30 | Prompts: 41
```

**Columns:**
- `#` - Block number
- `Started` - When the work block started
- `Stopped` - When the work block ended
- `Duration` - Total work time in minutes
- `Visual bar` - Relative duration visualization
- `Prompts` - Number of real user prompts
- `Avg Gap` - Average time between prompts

## Configuration

### Timeout Threshold

The default timeout is 10 minutes. To change it, edit the `TIMEOUT_MIN` variable:

```python
TIMEOUT_MIN = 10  # Change to your preferred value
```

**How to determine the right timeout:**
- Analyze your gap patterns with the gap_analysis.py script
- Look for the natural break point between "still working" and "took a break"
- Most gaps during active work are < 1 minute (Claude responding)
- Gaps > 10-15 minutes usually indicate you stepped away

### Timezone

Always specify your local timezone for accurate reporting:

- `--tz -3` for GMT-3 (Brazil)
- `--tz -5` for EST
- `--tz -6` for CST (Austin)
- `--tz 0` for UTC
- `--tz 1` for CET
...
- Session data is stored locally in `~/.claude/projects/`
- Each chat session creates a separate `.jsonl` file
- Timestamps are stored in UTC and converted to local time for display
- Tool results (Jira, file reads, etc.) are filtered out from prompt counts
